...
-- Mirror of migration 047_products.sql. `products` is syncable (Dexie
-- mirror + outbox create); `jobs.products_used` is JSONB riding the jobs
-- sync. The completion constraint is relaxed so ZERO products is valid
-- (survey visits) — this predicate MUST match isServiceSheetFilled +
-- ServiceSheetSchema exactly. See the migration for full rationale.
-- ============================================================
...
-- Relaxed completion backstop: ZERO products is valid (survey visits). Product
-- requirement DROPPED. MUST match isServiceSheetFilled + ServiceSheetSchema.
alter table jobs drop constraint if exists jobs_completed_requires_filled_sheet;
alter table jobs add constraint jobs_completed_requires_filled_sheet
  check (
    job_status <> 'completed'
