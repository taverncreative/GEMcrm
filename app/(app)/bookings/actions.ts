...
// ─── Upgrade a draft to a real booking (Q3) ──────────────────────────────

/**
 * Upgrade a DRAFT job to a real scheduled booking.
 *
 * Reuses createQuickBookingAction's customer → site resolution VERBATIM
 * (existing/new customer, existing/new site, client-UUID injection for
 * deterministic offline replay), then swaps the final step: instead of
 * createBooking (INSERT) it calls {@link upgradeDraftJob} (a guarded
 * UPDATE on the existing draft row, addressed by `draft_job_id`). The
 * new-customer/new-site rows are created here BEFORE the job UPDATE, so
 * the proven FK ordering is inherited untouched.
 *
 * On a partial-unique clash it returns the SAME
 * `{ success:false, errors:{ job_date }, message }` shape createBooking's
 * callers use, so the clash flows through the existing conflict-inbox
 * surfacing (classifyActionResult builds the human message from errors).
 *
 * The resolution blocks are copied — not extracted into a shared helper —
 * deliberately: createQuickBookingAction is live in production and the
 * brief is to leave its (and createBooking's) insert path byte-for-byte
 * unchanged.
 */
export async function upgradeDraftToBookingAction(
  _prev: ActionState,
  formData: FormData
): Promise<ActionState> {
  await requireUser();

  const draftJobId = (formData.get("draft_job_id") as string) || "";
  if (!draftJobId) {
    return { success: false, errors: {}, message: "Missing draft job id" };
  }
  ...
  // Offline-first id-injection — same as createQuickBookingAction: the
  // client UUIDs applyLocal already wrote to Dexie, so server rows match.
  const customerIdNew = (formData.get("customer_id_new") as string) || undefined;
  const siteIdNew = (formData.get("site_id_new") as string) || undefined;

  // Step 1 — resolve customer (verbatim from createQuickBookingAction)
  let customerId = data.customer_id;
  try {
    if (data.mode_customer === "new") {
      const customerResult = CustomerSchema.safeParse({
  ...
  // Step 2 — resolve site (verbatim from createQuickBookingAction)
  let siteId = data.site_id;
  try {
  ...
  // Step 3 — upgrade the draft (the ONLY divergence from
  // createQuickBookingAction, which inserts a new job here instead).
  ...
  try {
    await upgradeDraftJob(draftJobId, bookingResult.data);
  } catch (err) {
    if (err instanceof JobClashError) {
      return {
        success: false,
    ...
    return {
      success: false,
      errors: {},
      message: err instanceof Error ? err.message : "Failed to upgrade draft",
    };
  }

  // Freshen everywhere the now-real booking shows (and the draft leaves).
  revalidatePath(ROUTES.DASHBOARD);
  revalidatePath(ROUTES.JOBS);
  revalidatePath(ROUTES.CALENDAR);
  revalidatePath(ROUTES.CUSTOMERS);
  revalidatePath(ROUTES.siteDetail(siteId));
  revalidatePath(ROUTES.customerDetail(customerId));
  revalidatePath(ROUTES.jobDetail(draftJobId));

  return { success: true, errors: {}, message: "Draft upgraded" };
}
